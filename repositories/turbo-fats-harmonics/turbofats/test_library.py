...
def white_noise():
    ...
    #error = np.random.normal(loc=0.0, scale=0.01, size=10000)
    #time2 = np.arange(0, 10000)
    ...
    #error2 = np.random.normal(loc=0.0, scale=0.01, size=10000)
    aligned_time = time
    aligned_time2 = time2
    aligned_mag = mag
    aligned_mag2 = mag2
    ...
    #lc = np.array([data, mjd, error, second_data, aligned_data, aligned_second_data, aligned_mjd])#KP: This creates an error in Stetson

	#--------------------------------------------------------------------------------------------------------------
	#KP: According to the feature functions:
	#--------------------------------------------------------------------------------------------------------------
	# magnitude = data[0]
	# time = data[1]
	# error = data[2]
...
def test_FluxPercentile(white_noise):
    ...
    a = FeatureSpace(featureList=['FluxPercentileRatioMid20', 'FluxPercentileRatioMid35', 'FluxPercentileRatioMid50',
